...
// Define your data interface with new fields
interface Data {
  id: number;
  name: string;
...
const headCells: readonly HeadCell[] = [
  ...
  { id: 'percentOfCustomers', numeric: true, disablePadding: false, label: '% of Customers' },
    ...
    <TableHead>
      <TableRow>
        ...
        {headCells.map((headCell) => (
          ...
          >
            {headCell.id === 'actions' ? (
              /* For actions column: no sort label */
              headCell.label
            ) : (
              <TableSortLabel
...
export function SegmentsTable({ rows }: SegmentsTableProps) {
  ...
  const handleActionsClick = (event: React.MouseEvent<HTMLElement>, id: number) => {
    event.stopPropagation();
    setAnchorEls((prev) => ({ ...prev, [id]: event.currentTarget }));
  };

  const handleActionsClose = (id: number) => {
    setAnchorEls((prev) => ({ ...prev, [id]: null }));
  };

  const handleExport = (id: number) => {
    console.log(`Export row ${id}`);
    handleActionsClose(id);
  };

  const handleRename = (id: number) => {
    console.log(`Rename row ${id}`);
    handleActionsClose(id);
  };
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell align="right">
                      {row.percentOfCustomers.toFixed(1)}%
                    </TableCell>
                    ...
                    <TableCell align="left">
                      ...
                      <Menu
                        ...
                        onClose={() => handleActionsClose(row.id)}
                        onClick={(e) => e.stopPropagation()}
                      >
                        <MenuItem onClick={() => handleExport(row.id)}>Export</MenuItem>
                        <MenuItem onClick={() => handleRename(row.id)}>Rename</MenuItem>
